...
$app->post('/api/MicrosoftVideo/detectMotion', function ($request, $response) {
    ...
    $optionalParams = ['sensitivityLevel'=>'sensitivityLevel','frameSamplingValue'=>'frameSamplingValue','detectionZones'=>'detectionZones','detectLightChange'=>'detectLightChange','mergeTimeThreshold'=>'mergeTimeThreshold'];
    $bodyParams = ['url'];
    ...
    $query_str = "https://westus.api.cognitive.microsoft.com/video/v1.0/detectmotion";
    ...
    foreach ($optionalParams as $item){
        if(!empty($post_data['args'][$item])){
            $queryParams[$item] = $post_data['args'][$item];
        }
    }
